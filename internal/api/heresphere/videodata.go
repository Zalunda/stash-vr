...
func setMediaSources(vd *library.VideoData, dto *videoDataDto, currentFileId string) {
	files, labels := vd.GetFilesSortedByLabel()
	if len(files) == 0 {
		return
	}

	// Grab the direct stream just to extract the base URL and API keys/auth params generated by stash-vr
	baseStream := stash.GetDirectStream(vd.SceneParts)
	if len(baseStream.Sources) == 0 {
		return
	}
	originalUrl := baseStream.Sources[0].Url

	// Iterate through each file and create a SEPARATE mediaDto (Encoding group) for each
	firstFile := false
	for _, file := range files {
		...
		// Build the URL targeting your new custom Stash API route
		newPath := fmt.Sprintf("/file/%s/stream", fId)
		uniqueUrl := strings.Replace(originalUrl, "/stream", newPath, 1)

		// 1. Put the Label in the Media Name (e.g., "Part A" or "Direct" if only one file)
		mediaName := "Direct"
		...
		// 2. Put the actual height in the Resolution (strictly as an int)
		resHeight := int(file.Height)
